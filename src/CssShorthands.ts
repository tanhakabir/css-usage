...
export default class CssShorthands {
    /**
	...
	private static expandCache = Object.create(null);
	private static unexpandCache = Object.create(null);
     ...
     */
    public static expand(property: string): any {
		
		var result = CssShorthands.expandCache[property];
		if(result) { return result; }
		
		var prefixData = property.match(/^(-[a-zA-Z]+-)?(.*)$/);
		var prefix = prefixData[1]||'', prefixFreeProperty = prefixData[2]||'';
        if (!CssShorthands.shorthands.hasOwnProperty(prefixFreeProperty)) {
            return [];
        }
		
		result = [];
        CssShorthands.shorthands[prefixFreeProperty].forEach((p) => {
            var longhand = p[0] === '-' ? property + p : prefix + p;
            result.push(longhand);
			result.push.apply(result, this.expand(longhand));
        });
		
		return CssShorthands.expandCache[property] = result;
    };
	 ...
	 */
	public static unexpand(property: string): any {
		var result = CssShorthands.unexpandCache[property];
		if(result) { return result; }
		...
		result = [];
		for(var sh = 0; sh <= CssShorthands.shorthands.length; sh++) {
            var shorthand = CssShorthands.shorthands[sh];
		...
		return CssShorthands.unexpandCache[property] = result;
	}
}