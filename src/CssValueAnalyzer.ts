export default class CssValueAnalyzer {

    /**
     * This will take a string value and reduce it down
     * to only the aspects of the value we wish to keep
     */
    public static parseValues(value: any, propertyName: any): string {
        ...
        if (CssValueAnalyzer.isKeywordColor(value)) { return "<color-keyword>"; }
        ...
        value = value.replace(
            ...
            s=>numbers.reduce(
                (m,nstr,nint)=>m.replace(RegExp(/\d/,'g'),nstr),
                s
            )
        );
     ...
     */ 
    public static createValueArray(value, propertyName) {
     ...
     * keyword color value
     */
    private static isKeywordColor(candidateColor) {
